...
| *"inter-agent routing logic failure-tolerant… if a worker agent loops or returns a hallucination"* | Hallucination is bounded: detection is deterministic and runs before any model call, so a fabricated finding has no binding behind it. Retry is no longer hand-rolled — `resilience.py` was deleted on 2026-08-15 and replaced on 2026-08-16 by managed Agent Runtime plus Eventarc five-attempt dead-letter delivery and Firestore lease reclaim, all deployed and exercised across the integration and load suites | ✅ Both implemented and deployed; the timeout → retry → escalate sequence is **not yet captured** as an observed artifact |
