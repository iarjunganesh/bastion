...
def test_a_loosened_threshold_is_reported():
    """A guardrail relaxed in the console still answers, so nothing else would notice."""
    drift = template_drift(deployed(confidenceLevel="HIGH"))
...
def test_the_threshold_still_refuses_a_high_confidence_injection():
    """Measured on the live template: the evidence 01 probe scores HIGH, the fleet's own
    instructions score LOW. Only a threshold strictly between them keeps both properties, so
    this pins the one setting that does."""
